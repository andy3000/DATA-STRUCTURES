...
int main ()
{
   ...
   // set time equal to zero
	...
	// if simLenght equal to zero don't start loop
    if( simLength != 0 )
    {
		...
		do
		{
		   ...
		   if( !custQ.isEmpty() )
		   {
			  ...
			  // calculate cusomter's wait time
			  waitTime = ( minute - timeArrived );
			  
			  // calculate longest customer wait
			  // if waitTime is greater than maxWait
			  if( waitTime > maxWait )
			  {
				  // assign new maxWait
				  maxWait = waitTime;
			  ...
			  // increment total cusomters served
			  totalServed++;	      
		   }
			  
		   // calculate how many customers to add to line
		   numArrivals = rand() % 4;
		   
		   // if new customers equal to one
		   if( numArrivals == 1 )
		   {
			  // add one customer to the line
			  custQ.enqueue( minute );
		   }   
		   // if new customers equal to two
		   if( numArrivals == 2 )
		   {
			  // add two customers to the line
			  custQ.enqueue( minute );
			  custQ.enqueue( minute );
		   }
		 
		}while( simLength > minute );
		// end loop 
	}
